...
class EquipmentBorrowing(TimestampMixin, Base):
    ...
    borrow_time: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    expected_return: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    actual_return: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
